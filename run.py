...
def run_step(opts, world_size, rank, device):
    ...
    new_cls, old_labels, path = tasks.get_task_labels(opts.dataset, opts.task, opts.step)
    print(new_cls)
    ...
    params = []
    if opts.freeze_all_but_new:
        for param in model.parameters():
            param.requires_grad = False

        for param in model.cls[-1].parameters(): # classifier for new class
    ...
    if opts.step > 0 and not opts.test:
        ...
        if os.path.exists(path):
            ...
            if opts.init_balanced:
                ...
                imp_path = f"channels/{att_method}/imp_{opts.method}_{opts.dataset}_{opts.task}_{opts.step}.pt"
                low_path = f"channels/{att_method}/low_{opts.method}_{opts.dataset}_{opts.task}_{opts.step}.pt"
                if os.path.exists(imp_path) and opts.att != 0:
                    imp_c = torch.load(imp_path).to(device)
                else:
                    imp_c = None
                    print("imp_c is None")
                ...
                # implement the AWT initialization
                print('Classifier Initialization done....')
                if not opts.no_att:
                    if not opts.copy:
                        if opts.random_channels:
                            f = [x for x in range(256)]
                            rr = random.sample(f, opts.rand)
                            imp_c = torch.zeros(256, 1, 1).cuda()
                            imp_c[rr] = 1.0
                            print('Using random channels for weight transfer', imp_c.sum())

                        model.module.init_new_classifier(device, imp_c, copy_bias=opts.copy_bias, orig_init=opts.orig_init)
                    if opts.copy:
                        model.module.init_new_classifier(device, imp_c, copy=opts.copy, copy_share=opts.copy_share)
    ...
    model.eval()
    #model.module.last_layer_stats(device)
