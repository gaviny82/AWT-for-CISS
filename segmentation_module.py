...
class IncrementalSegmentationModule(nn.Module):
    ...
    def _network(self, x, ret_intermediate=False, only_bg=False):
        #self.model.apply(cast_running_stats)
        ...
        if self.use_cosine:
            ...
            #x_clf = F.normalize(x_pl, dim=1, p=2)
        else:
            x_clf = x_pl
    ...
    def init_new_classifier(self, device, imp_c=None, copy_bias=False, copy=False, copy_share=0, orig_init=False):
        cls = self.cls[-1]
        ...
        if self.multi_modal_background:
            ...
            if not self.use_cosine:
                ...
                print('Bkg bias ',bkg_bias)

        if orig_init:
            cls.weight.data.copy_(imprinting_w)
            #cls.weight.data += imprinting_w

        if not self.use_cosine:
            bias_diff = torch.log(torch.FloatTensor([self.classes[-1] + 1])).to(device)
            new_bias = (bkg_bias - bias_diff)

        if imp_c is not None and not copy:
            cls.weight.data += imprinting_w*(imp_c).float()
            ...
            cls.weight.data.copy_(imprinting_w*(imp_c).float()+cls.weight.data*(~imp_c))
            print('bkg classifier weights copied to new classifier weights')
        ...
        if orig_init:
            ...
            if self.multi_modal_background:
                ...
                if not self.use_cosine:
                    self.cls[0].bias[0].data.copy_(new_bias.squeeze(0))
            print("Original initialization done")
