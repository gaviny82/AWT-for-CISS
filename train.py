        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            if self.pseudo_soft is not None:
                ...
                if self.ce_on_pseudo and self.step > 0:
                    ...
                    #loss = criterion(outputs, labels, outputs_old)  # B x H x W  used in MiB
                    loss = criterion(outputs, labels)  # used in PLOP
            else:
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(loader):
                ...
                if not self.icarl_only_dist:
                    if self.model_old is not None:
                        #loss = criterion(outputs, labels, outputs_old)  # B x H x W
                        loss = criterion(outputs, labels)  # used in PLOP
                    else:
...
def features_distillation(
    ...
    :return: A float scalar loss.srun -K --ntasks=1 --gpus-per-task=2 -p RTX6000 --cpus-per-gpu=2 --mem-per-cpu=10GB --container-mounts=/netscratch/$USER:/netscratch/$USER,/ds-av:/ds-av,"`pwd`":"`pwd`" --container-image=/netscratch/enroot/dlcc_pytorch_20.05.sqsh --container-workdir="`pwd`" bash install.sh
    """
